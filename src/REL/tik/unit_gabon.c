...
/*.mBaseDamageParams = */       {GABON_ATTACK_DAMAGE,
                                0,
                                0,
                                0,
                                0,
...
static EVT_BEGIN(attack_event) {
    ...
    ELSE
        ...
        CALL(btlevtcmd_SamplingEnemy, -2, LVar0, PTR(&weapon_tik_gabon_attack))
        CALL(btlevtcmd_ChoiceSamplingEnemy, PTR(&weapon_tik_gabon_attack), LVar3, LVar4)
    END_IF
    IF_INT_EQ(LVar3, -1)
    ...
    CALL(btlevtcmd_WeaponAftereffect, PTR(&weapon_tik_gabon_attack))
